...
\newacronym{SMTP}{Simple Mail Transfer Protocol}{description={
    SMTP, originally defined in \RFC{}~821~\cite{RFC821} and updated in
    \RFC{}~2821~\cite{RFC2821}, is the protocol which transfers mail from
    the sender to the recipient across the Internet.  It is a simple, human
    ...
    scope of this document: introductory guides can be found
    at~\cite{smtp-intro-01, smtp-intro-02}, the definitive references
    are~\cite{RFC821, RFC2821}.
    %XXX A brief introduction to SMTP is
    %provided in \sectionref{SMTP background}.
}}
...
\newacronym{regex}{Regular Expression}{description={
    Regular expressions are a method of matching patterns against text that
    are explained in detail in~\cite{Wikipedia-regex, perlre}.
}}
...
\newacronym{NLP}{Natural Language Processing}{description={
    ...
    can understand human languages.  It is in no way connected to
    Neuro-linguistic programming, which is generally found when searching
    for NLP.
}}
