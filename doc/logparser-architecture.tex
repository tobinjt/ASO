...
is certainly not a task to be performed every time a parser runs: naive
...
XXX CARL SUGGESTS COMPARING THIS TO THE POWERSET\@; IS THAT REASONABLE\@?
...
are divided into sets based on how their conditions overlap, the complexity
of detecting overlap amongst $n$ rules is:

XXX IN THE FORMULA BELOW $s \in{} overlaps$ SHOULD BE UNDER $\sum$

$$O(n^{2}y) + O\left(\sum{|s|^{2}x \forall{} s \in{} overlaps}\right)$$
...
for condition overlap only between rules that have overlapping regexes.
