...
in Yet More Aborted Delivery Attempts (\sectionref{yet more aborted
delivery attempts}).  The more intrusive the changes are, the more likely
...
require; \sectionref{rules table} describes the attributes used in
...
rules are applied in the order listed in \tableref{Rules to parse C-style
comments} because rule two overlaps with rule one, and rule four overlaps
with rule three; \sectionref{rules in architecture} has explained how this
is achieved.  \sectionref{overlapping rules in architecture} will discuss
...
equivalent in computational power to \acronym{FA} and can be converted to
...
frequently recognise inputs (see \sectionref{rule ordering for
efficiency}), then a rule with a regex that matches inputs it should not
...
but if a regex is converted to a \acronym{FA} or the internal
