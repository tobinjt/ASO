...
XXX UPDATE SECTION REFERENCES BELOW\@: THEY SHOULD ONLY REFER TO THE
ARCHITECTURE, THE IMPLEMENTATION CAN REFER BACK TO HERE\@.
...
\begin{eqlist}
    ...
    \item [Framework]  The framework is the structure that actions and
        ...
        other support functions.  The framework is described in detail in
        \sectionref{framework in architecture}.
    ...
    \item [Rules]  The rules are responsible for classifying inputs,
        ...
        inputs and extracts data.  Rules provide an easily extensible
        method of associating log lines with actions, and are described in
        detail in \sectionref{rules in architecture}.
...
made by the developer (\sectionref{adding new actions in implementation}
describes safely adding new actions).  \parsername{} allows the user to add
new rules to the database without changing the parsing algorithm, unless
the new log lines to be parsed require functionality not already provided
by the algorithm.  If the new log lines do require new functionality, new
actions can be added to the parser without modifying existing actions or
other parts of the algorithm; only in the rare event that the new actions
...
\acronym{ATN}~\cite{atns,nlpip}, a tool used in Computational Linguistics
for creating grammars to parse or generate sentences.  The resemblance
between \acronym{ATN} and the parser is accidental, but it is obvious that
the two different approaches share an underlying separation of concerns;
...
XXX EXTEND FRAMEWORK SECTION IF NECESSARY\@.
