...
view of the log files that future tools can utilise.  The gathered data
...
changes will not cause the user to be negatively affected.  Statistics
and graphs can be generated from the database; new restrictions can be
...
that need to interact with the data in several ways:

\begin{enumerate}

    \item It is already possible to access databases from most programming
...
\end{verbatim}
    ...
    \item \SQL{} is reasonably standard and many people will already be
        ...
        implementation) several are available~\cite{sqlite-guis}.
...
protocol, making it quite simple to test and debug problems with it.  Despite
...
sending programs that are this deficient --- unfortunately others
...
command, or accepting sender or recipient addresses that were not enclosed
in \texttt{<>}.  
...
remaining minority can be quite complex (see \sectionref{additional
...
the name of the Postfix component that produced it, and this information
...
positive security implications also: those components that interact with
...
address.\footnote{E.g.\ the administrator may require that all clients on
...
all users --- the administrator must be cognisant of the effects of their
...
stupid things, as that would also stop them from doing clever things''\/}
--- Doug Gwyn~\cite{unix-philosophy}.

Postfix uses simple lookup tables to make decisions when evaluating some
restrictions, e.g.\ in the example restriction below\newline
...
of the connected client is found in the file and returns the associated
action if found; the method of searching the file is dependant on the type
of the file (\texttt{cidr} in the example) --- see
...
\begin{itemize}
    ...
    \item Checking \SPF{} records~\cite{openspf, Wikipedia-spf}.
        ...
        joe~jobs~\cite{Wikipedia-joe-job}, but there has been considerable
    ...
    \item Greylisting~\cite{greylisting} is a technique that temporarily
        ...
        particularly when you consider Greylisting's position as the final
        restriction that a mail must overcome:\footnote{Greylisting is the
        ...
        Greylisting only takes effect for mails that have passed every
        other restriction.
    ...
    \item Using a scoring system such as
        ...
        they are to bypass a restriction; using a threshold that requires a
        delivery attempt to hit several restrictions frees the
        administrator from whitelisting clients that fall foul of one
        restriction only.
