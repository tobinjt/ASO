...
typical \acronym{SMTP} conversation resembles the following (the lines
...
phishing mails using faked sender addresses.  For example, a phishing mail
might pretend that the payroll system had a disastrous disk failure, and
until the server is replaced all salary payments will have to be processed
...
\begin{itemize}
    ...
    \item \acronym{SPF} checks:
        \urlLastChecked{http://www.openspf.org/}{2009/04/21}.
    ...
    \item Greylisting,
        ...
        \sectionref{Evaluation} are shown in \tableref{Most effective
        Postfix restrictions}.  The table shows that greylisting is worth
