...
\begin{enumerate}
    ...
    \item If one or more recipients have been accepted, Postfix will have
        ...
        last log line from \daemon{smtpd} for that connection; this depends
...
parsing the \daemon{cleanup} log line causes the CLEANUP\_PROCESSING action
to create a new state table entry, to help deal with re-injected mails
(\sectionref{Re-injected mails}).  This is incorrect because the log line
...
During the TIMEOUT action, if the \daemon{cleanup} line is still pending,
...
CLEANUP\_PROCESSING action blindly discarded the next \daemon{cleanup} log
line after a timeout it would sometimes be mistaken.  When the next
\daemon{pickup} log line containing that queueid is processed the queueid
will be removed from the cache of timeout queueids, regardless of whether
it meets the criteria above.

This complication is handled by the CLEANUP\_PROCESSING and TIMEOUT
actions.
...
log file than the \daemon{cleanup} log line, so the PICKUP action will find
