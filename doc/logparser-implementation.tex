...
plus a description of the \acronym{regex} snippets provided to ease the
...
submission via \daemon{sendmail} or \daemon{postdrop}, by \acronym{SMTP},
...
immediately \flowchart{PICKUP}{2}, and the mail moves on to the delivery
stage.  Re-injection because of forwarding lacks explicit log lines of its
own; it is explained fully in \sectionref{Re-injected mails}.  Internally
generated mails lack any explicit origin in Postfix 2.2.x and must be
detected using heuristics as described in \sectionref{identifying bounce
notifications}; later versions of Postfix do provide log lines for
internally generated mails \flowchart{BOUNCE\_CREATED}{3}.  Bounce
...
\begin{enumerate}
    ...
    \item This is followed by rejection of one or more mail delivery
        attempts \flowchart{DELIVERY\_REJECTED}{4}; acceptance of one or
        more mails \flowchart{CLONE}{5}; failure of the remote client's
        connection \flowchart{DELIVERY\_ERROR}{6}; or some random
...
Each rule has a \acronym{regex} to recognise log lines and capture data
...
\sectionref{rules in implementation}, but the rules table is documented in
\sectionref{rules table} with the rest of the database schema.
...
\end{verbatim}
    ...
    \item \acronym{SQL} is reasonably standard and many people will already
        be familiar with it; for those unfamiliar with \acronym{SQL}, lots
        of resources are available from which to learn, e.g.\
...
\begin{boldeqlist}
    ...
    \item [regex] The \acronym{regex} to recognise log lines with, as
        documented in \sectionref{regex components}.
    ...
    \item [hits] This counter is maintained for every rule and incremented
        ...
        for efficiency is discussed in \sectionref{rule ordering for
        efficiency}.  This field will be set by the parser rather than the
        rule author.
...
\begin{itemize}
    ...
    \item That the \acronym{regex} is valid.  The \acronym{regex} will have
        \acronym{regex} components expanded (see \sectionref{regex
        components}), and will also be compiled for efficiency
        (\sectionref{Caching compiled regexes}).

    \item For overlap between the data captured by the \acronym{regex} and
        additional data specified in either \texttt{connection\_data} or
        \texttt{result\_data}.

    \item That \texttt{connection\_data}, \texttt{result\_data}, and the
        \acronym{regex} captures specify valid fields to save data to.
...
\begin{enumerate}
    ...
    \item Each \acronym{regex} is compiled and the result cached when the
        ruleset is loaded; this optimisation can be disabled and the
        \acronym{regex} compiled every time it is used when recognising log
...
\begin{enumerate}
    ...
    \item Every time a log line is recognised, the recognising rule's
        \acronym{regex} and the log line can be printed, so that the user
...
files, excluding log files 22 \& 62--68 because their contents are
extremely skewed by mail loops}{Number of times each action was invoked
excluding 22 and 62--68}
...
rule's \acronym{regex} to the connection or mail; usually, log lines that
lack identifying information will be processed by the
\action{UNINTERESTING} action.  Each action is passed the same arguments:
...
\begin{boldeqlist}
    ...
    \item [line] The log line, separated into fields:
        ...
        \begin{boldeqlist}
            ...
            \item [pid] The \acronym{pid} of the process that generated the
                log line.
...
\begin{description}
    ...
    \item [CLEANUP\_PROCESSING] \daemon{cleanup} processes every mail that
        ...
        by the rule's \acronym{regex} if the log line has not come after a
        timeout (see \sectionref{discarding cleanup log lines}); it also
    ...
    \item [DELIVERY\_REJECTED] Postfix rejected a mail delivery attempt
        ...
        \acronym{regex} to the mail identified by that queueid; otherwise
        save it to the connection identified by the \acronym{pid} in the
        log line.
    ...
    \item [MAIL\_BOUNCED] This action behaves in exactly the same way as
        ...
        recognising rule's \acronym{regex}, and does nothing more.  It is a
    ...
    \item [MAIL\_DISCARDED] Sometimes mail is discarded by Postfix, e.g.\
        mail submitted locally on the server using \daemon{pickup} that is
        larger than the limit configured by the administrator.  This action
        is used for those mails; it invokes the \action{COMMIT} action, but
        is a separate action to simplify further analysis.
    ...
    \item [MAIL\_SENT] This action behaves in exactly the same way as the
        ...
        recognising rule's \acronym{regex}, and does nothing more.  It is a
        separate action to distinguish successful delivery attempts from
        other delivery attempts.

    \item [MAIL\_TOO\_LARGE] When a remote client tries to send a mail over
        \acronym{SMTP} that is larger than the server accepts, the mail
        will be discarded by Postfix and the client informed of the
        problem.  The mail may have been accepted and partially
        transferred; if so the parser will have a data structure that must
        be disposed of.  See \sectionref{timeouts during data phase} for
        full details; although that describes timeouts, the processing is
        the same for mails that are too large.
    ...
    \item [SAVE\_DATA] Every action that can locate the correct data
        ...
        recognising rule's \acronym{regex} to it.  The \action{SAVE\_DATA}
...
that creates \regexes{} from unparsed log lines; the \acronym{regex}
components provided by \parsername{} to ease writing of complex \regexes{}
are covered in \sectionref{regex components}.  The rule conditions used in
\parsername{} are the penultimate topic (\sectionref{rule conditions in
implementation}), and this section concludes with some suggestions for how
...
\begin{description}
    ...
    \item [program and regex:] program is used to restrict the log lines
        ...
        conditions in implementation} for details.  The \acronym{regex}
        does the actual recognition of log lines, and data captured by the
        \acronym{regex} (e.g.\ sender, recipient) will be automatically
        saved to the results and connections tables.  The \acronym{regex}
        components used in the \acronym{regex} are described in
...
description, program, and \acronym{regex}; all the other attributes are
...
something else; examine the list of actions in \sectionref{actions in
detail in implementation} to determine the correct one.  The
\acronym{regex} needs to be based on the log line, but see below for a tool
...
generate a \acronym{regex} to match each group of log lines.  The algorithm
has five steps:
...
\begin{description}
    ...
    \item [Build regexes.]  The words are reassembled to produce a
        \acronym{regex} matching the log line, and a counter is maintained
        for each \acronym{regex}.  Contiguous sequences of \texttt{.+} in
        the newly assembled \regexes{} are collapsed to a single
        \texttt{.+}; any resulting duplicate \regexes{} and their counters
        are combined.  If a \acronym{regex}'s counter is lower than the
        threshold supplied by the user the \acronym{regex} is discarded;
    ...
    \item [Test the new regexes.]  The final step replaces keywords in the
        ...
        each \acronym{regex} to check they are valid.  A match will be
        ...
        \acronym{regex}, or if a log line is matched by more than one
        \acronym{regex}.  The number of log lines each \acronym{regex}
        ...
        by more than one \acronym{regex}.
...
displayed are the number of unrecognised log lines each \acronym{regex} was
...
Each rule's \acronym{regex} will have keywords expanded when the ruleset is
loaded, for several reasons:

\begin{itemize}

    \item It simplifies both reading and writing of \regexes{} and helps to
        make each \acronym{regex} largely self-documenting.  For example,
    ...
    \item It avoids needless repetition of complex \acronym{regex}
    ...
    \item It enables automatic extraction and saving of captured data.  The
        \acronym{regex} snippets use Perl 5.10's named capture
...
\acronym{regex} is compiled before attempting to parse the log file,
otherwise every \acronym{regex} would be recompiled each time it was used,
resulting in a large, data dependent slowdown, as described in
\sectionref{Caching compiled regexes}.  Most of the keywords are named
...
Some similarity exists between \acronym{regex} components and cascaded
parsing: each \acronym{regex} component resembles a rule that recognises
...
two is that \acronym{regex} components are explicitly used by the author of
...
\begin{itemize}
    ...
    \item Sort rules by program and \acronym{regex} e.g.\ with an
        \acronym{SQL} query similar to:                 \newline{}
        \verb!SELECT program, regex!                    \newline{}
        \verb!    FROM rules!                           \newline{}
        \verb!    ORDER BY program, regex;!             \newline{}
        The rules are sorted first by program, then by \acronym{regex},
...
% Move section start onto next page because extra spacing causes it to be
% badly positioned.
\newpage{}
...
accompanied by a description of the \acronym{regex} snippets the framework
