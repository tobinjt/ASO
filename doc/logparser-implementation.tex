...
data phase}).  \parsername{} is capable of parsing log files from Postfix
...
\flowchart{BOUNCE\_CREATED}{3}.  Bounce notifications are the primary
...
\begin{boldeqlist}
    ...
    \item [restriction\_name] The name of the Postfix restriction that
        ...
        action of \action{DELIVERY\_REJECTED}.
...
measure the effect they have.  In each case the data gathered with the
optimisation disabled will be compared to the data gathered with it
enabled, to quantify the benefit each optimisation provides.  Three sets of
data are collected:
...
(\action{CLEANUP\_PROCESSING}), and again there is only a 0.01648\%
...
components \parsername{} currently does not have rules for, e.g.\
