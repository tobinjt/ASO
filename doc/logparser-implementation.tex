...
architecture described in \sectionref{parser architecture}.  Any design may
...
\sectionref{complications}) should help if someone needs to develop a
...
log lines (e.g.\ \sectionref{aborted delivery attempts}), and, less
frequently, to discard a data structure (e.g.\ \sectionref{timeouts during
data phase}).  \parsername{} can parse log files from Postfix version 2.2
through to version 2.5, and should be capable of parsing log files from
later versions with only minor modifications or an updated ruleset.  The
parser's task is to follow the journey each mail takes through Postfix,
combining the data captured by rules into a coherent whole, saving it in a
useful and consistent form, and performing housekeeping duties.  The
intermingling of log lines from different mails immediately rules out the
possibility of handling each mail in isolation; the parser must handle
...
states and transitions shown in \figureref{flow chart image} will be
...
\begin{enumerate}
    ...
    \item This is followed by rejection of \acronym{SMTP} commands by the
        restrictions configured by the administrator
...
\begin{enumerate}
    ...
    \item Sometimes mail is re-injected and the child mail needs to be
        tracked with the parent mail \flowchart{TRACK}{9}; the handling of
...
to a remote server, bounced (because of a mail loop, delivery failure, or
five day timeout), or deleted by the administrator, \textit{unless\/} the
mail is either parent or child of re-injection, as explained in
...
further applications that utilise the data gathered; \sectionref{database
...
mail accepted and every connection that rejected an \acronym{SMTP} command;
the individual fields will be described in \sectionref{connections table}.
The results table will have at least one entry for each entry in the
...
created it.  No ambiguity or confusion is possible about which rule
resulted in a particular result, eliminating one potential source of
confusion.
...
\acronym{API}: it provides a stable interface between the user and the
...
\sectionref{motivation} to far more complex data mining tools.
...
will not be modified by \parsername{} (except for the hits and hits\_total
...
take for each recognised log line.

Each rule some or all of these fields; all fields are required unless
otherwise stated.
...
\begin{boldeqlist}
    ...
    \item [description] Something must have occurred to cause Postfix to
        output each log line; this field should describe the event causing
        the log lines this rule recognises.

    \item [restriction\_name] The name of the Postfix restriction that
        caused the mail to be rejected.  This field is valid only for rules
        that recognise rejection log lines, i.e.\ rules that have an action
        of \action{DELIVERY\_REJECTED}.
    ...
    \item [priority] This is the user-configurable companion to hits: when
        ...
        described in \sectionref{rules in architecture}.  This field is
        optional.
...
Every accepted mail and every connection that rejected an \acronym{SMTP}
command will have a single entry in the connections table containing all of
...
\begin{boldeqlist}
    ...
    \item [server\_ip] The IP address of the server: the local address when
        receiving mail, the remote address when sending mail.

    \item [server\_hostname] The hostname of the server, it will be
        \texttt{unknown} if the IP address could not be resolved to a
        hostname via DNS\@.

    \item [client\_ip] The client IP address: the remote address when
        receiving mail, the local address when sending mail.
...
\begin{boldeqlist}
    ...
    \item [warning] Administrators can configure Postfix to log a warning
        ...
        \acronym{SMTP} command --- a mechanism that is quite useful for
...
lines processed by the \action{SAVE\_DATA} action can be discarded
...
\begin{itemize}
    ...
    \item That the regex is valid.  The regex will also be compiled for
        efficiency (\sectionref{Caching compiled regexes}).
...
as the relationships between mails must be maintained; loading state is
also complicated by dealing with aborted delivery attempts
...
a variable number of log lines; variation between the recognition and
processing time of individual log lines also affects its accuracy, but
overall the progress bar is a useful addition.  \parsername{} is primarily
intended for parsing complete log files, but with minor modifications it
could be used to parse a live log file, periodically checking it for new
log lines; this could be very useful for programs that work best with up to
date data, e.g.\ a program for monitoring the health of the mail system.
...
\begin{enumerate}
    ...
    \item Each regex is compiled and the result cached when the rules are
        ...
        optimisation is shown in \sectionref{Caching compiled regexes}.
...
\begin{enumerate}
    ...
    \item Each result can be extended with extra debugging information,
        ...
        The information that is added is: the log line's timestamp in human
...
that is saved to the database, and sufficient safeguards are in place that
there should be plenty of warning messages explaining what is wrong without
using this option.
...
are recognised by a single rule and handled by the \action{CONNECT} action.
...
\sectionref{parser efficiency}, \textit{excluding\/} log files 22 and
...
\action{CLEANUP\_PROCESSING} is invoked more often than
\action{MAIL\_QUEUED}: occasionally, accepted mail in the process of being
...
files, excluding log files 22 and 62--68}{Number of times each action was
...
rule's regex.  Each action is passed the same arguments:
...
\begin{description}
    ...
    \item [DELIVERY\_REJECTED] Postfix rejected an \acronym{SMTP} command
    ...
    \item [MAIL\_TOO\_LARGE] When a client tries to send a mail larger than
        ...
        partially transferred; if so a data structure must be disposed of.
        See \sectionref{timeouts during data phase} for full details;
        although that describes timeouts the processing is the same for
        mails that are too large.

    \item [PICKUP] The \action{PICKUP} action corresponds to the
        \daemon{pickup} service dealing with a locally submitted mail.  A
        new mail will be created, although out of order log lines may have
        caused it to already exist, as documented in \sectionref{pickup
    ...
    \item [POSTFIX\_RELOAD] When Postfix stops or reloads its
        ...
        \daemon{smtpd} processes exiting.
    ...
    \item [SMTPD\_DIED] Sometimes a \daemon{smtpd} dies, is killed by a
        ...
        means that the small amount of data that has been gathered will be
        lost.
    ...
    \item [TIMEOUT] The connection with the remote client timed out, so the
        mail being transferred must be discarded.  The mail may have been
        accepted: if so there is a data structure to dispose of.  See
...
is dealt with in \sectionref{creating new rules in implementation}; the
...
from actions: adding new actions can be difficult, but adding new rules to
...
rule recognising the \daemon{pickup} component processing a mail sets the
...
\begin{description}
    ...
    \item [Pre-process the file.]  The new algorithm leverages the
        ...
        it performs many substitutions on the input log lines, replacing
        commonly occurring variable terms (e.g.\ email addresses, IP
        addresses, the standard start of rejection messages) with keywords
        described in \sectionref{regex components}.  The purpose of this
        step is to utilise existing knowledge to create more accurate
        regexes; it replaces many variable words with fixed words,
        improving the subsequent classification of words as fixed or
        variable.  The altered log lines are written to a temporary file,
        which subsequent stages will use instead of the original input
        file.
...
The new regexes are displayed for the user to add to the database, either
...
slowdown, as described in \sectionref{Caching compiled regexes}.  Most of
the keywords are named after the fields in the connections or results
tables they populate: \_\_CLIENT\_HOSTNAME\_\_, \_\_CLIENT\_IP\_\_,
\_\_DELAY\_\_, \_\_DELAYS\_\_, \_\_ENHANCED\_STATUS\_CODE\_\_,
\_\_HELO\_\_, \_\_MESSAGE\_ID\_\_, \_\_QUEUEID\_\_, \_\_RECIPIENT\_\_,
...
\begin{itemize}
    ...
    \item Sort rules by program and regex, then visually inspect the list,
        e.g.\ with \acronym{SQL} similar to:    \newline{}
...
\acronym{API} is described, followed by a detailed description and a
