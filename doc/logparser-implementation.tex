...
overcome them, are documented in \sectionref{complications}.  This chapter
...
simplest case: a queueid is assigned immediately \flowchart{PICKUP}{2}, and
...
\begin{enumerate}
    ...
    \item Postfix will log the sender and recipient addresses separately
        \flowchart{SAVE\_DATA}{8}.

    \item Sometimes mail is re-injected and the child mail needs to be
        tracked by the parent mail \flowchart{TRACK}{9}; the handling of
    ...
    \item Eventually the mail will be delivered, bounced, or deleted by the
        administrator \flowchart{COMMIT}{11}.  This is the last log line
...
further applications which utilise the data gathered; \sectionref{database
...
\end{verbatim}
    ...
    \item \acronym{SQL} is reasonably standard and many people will already
        ...
        overall similarity and minor differences amongst Irish English,
        ...
        reduce or remove the requirement to know \acronym{SQL}.
...
\begin{boldeqlist}
    ...
    \item [restriction\_name] The name of the Postfix restriction that
        ...
        \action{DELIVERY\_REJECTED}; other rules should not set this field.
...
re-injected mail (\sectionref{Re-injected mails}) complicates the process
...
(see \sectionref{rule conditions in implementation}), falling back to
generic rules if necessary, and warning if the log line is unrecognised.
The rules are sorted for increased efficiency; see \sectionref{rule
ordering for efficiency} for details.  The repetitive nature of log files
...
for the results chapter (\sectionref{Results}); some of the techniques used
...
and handled by the \action{CONNECT} action.
...
\Graphref{Number of times each action was invoked} shows the number of
times each action was invoked when parsing the \numberOFlogFILES{} log
files used to generate the statistics in \sectionref{parser efficiency}.
The action most commonly specified by rules, \action{DELIVERY\_REJECTED},
...
actions, with \action{SAVE\_DATA} being invoked for a mere 0.0903\% of
recognised log lines.

The action invoked most often is \action{MAIL\_QUEUED}; there is only a
0.0051\% difference between it and \action{CLEANUP\_PROCESSING}, and a
further 0.5203\% drop to \action{COMMIT}.  The near equality of these
numbers is to be expected, because every mail delivered by Postfix will be
managed in the queue by \daemon{qmgr} (\action{MAIL\_QUEUED}), processed by
\daemon{cleanup} (\action{CLEANUP\_PROCESSING}), and removed from the queue
by \daemon{qmgr} (\action{COMMIT}); the reason for the small disparity
between \action{MAIL\_QUEUED} and \action{COMMIT} is mainly because mails
are sometimes manually deleted from the queue.  Similarly \action{CONNECT}
and \action{DISCONNECT} have a mere 0.0004\% difference between their
number of invocations, because every connection will have a matching
disconnection unless the \daemon{smtpd} process is killed for some reason.
Unexpectedly there were far fewer connections (\action{CONNECT}) than there
were mails in the queue (\action{MAIL\_QUEUED}); this is because of two
local mail loops (\sectionref{Explaining the peaks in log file size}),
which did not involve delivery via \acronym{SMTP}, and skew the total
numbers.
...
\showgraph{build/graph-number-of-action-invocations}{Number of times each
action was invoked when parsing \numberOFlogFILES{} log files}{Number of times
each action was invoked}

\newlength{\tabcolsepORIG}
\setlength{\tabcolsepORIG}{\tabcolsep}
\addtolength{\tabcolsep}{-1pt}

\showtable{build/include-per-file-action-invocation-numbers-table}{Mean and
standard deviation of the number of invocations of each action, for
different groups of log files}{Mean and standard deviation of the number of
invocations of each action, for different groups of log files}
...
\begin{description}
    ...
    \item [MAIL\_DISCARDED] Sometimes mail is discarded, e.g.\ mail
        ...
        \action{COMMIT} action, but is a separate action to simplify
        further analysis.
...
does not have rules for, e.g.\ \daemon{virtual} or \daemon{lmtp}.
...
\action{DELIVERY\_REJECTED}; hopefully the name of the restriction will be
