...
as painless as possible.  The implementer writes a subroutine that accepts
...
work is required from the implementer to integrate the action into the
...
parser's object oriented implementation by sub-classing it and implementing
...
XXX ONCE THIS PARAGRAPH HAS BEEN INTEGRATED AND EXPANDED UPDATE THE
REFERENCE IN ARCHITECTURE->RULES->ATTACHING CONDITIONS TO RULES\@.
...
parse each log line (see \sectionref{rules in implementation} for details).
...
\begin{description}
    ...
    \item [Pre-process the file]  The new algorithm leverages the knowledge
        ...
        pass it generated a hash~\cite{hash-functions}\glsadd{hash} from a
