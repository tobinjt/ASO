...
XXX FIGURE OUT WHEN TO USE ACTIONS AND WHEN TO USE ALGORITHM, PARSER,
ETC\@.
...
black box program based solely on its inputs and outputs.  XXX REFER TO
BLACK BOX PAPER\@?  There are some advantages to treating Postfix as a
black box during parser development:
...
must be detected using heuristics as described in \sectionref{identifying
bounce notifications}.  Bounce notifications are the primary example of
internally generated mails, though there may be other types.  Postfix may
generate mails to the administrator when it encounters configuration
errors, but such mails are presumably rare.
...
by each rule.  The rules are described in detail in \sectionref{rules in
implementation}, but the fields are covered with the rest of the database
...
in \sectionref{Diagram of the Database Schema} before the in-depth
...
\begin{boldeqlist}
    ...
    \item [program] The Postfix component (e.g.\ \daemon{smtpd}) whose log
        ...
        in \sectionref{attaching conditions to rules}.
    ...
    \item [regex] The regex to recognise log lines with.  The regex will
        ...
        large, data dependent slowdown, as shown in \sectionref{Caching
        compiled regexes}.
...
rejecting a mail delivery attempt; next is \texttt{SAVE\_DATA}, the action
responsible for processing informative log lines, supplementing the data
gathered from other log lines.  The third most common action is, perhaps
...
they need to add to or extend their ruleset.  The remaining actions have
only one or two associated rules: some are specialised actions required to
address a deficiency in the log files, or a complication that arises during
parsing; other actions will only ever have one log line variant, e.g.\ all
log lines showing that a remote client has connected are recognised by a
single rule and handled by the \texttt{CONNECT} action.  \parsernames{}
actions are described in detail in \sectionref{actions in detail in
implementation}, and \sectionref{adding new actions in implementation}
covers the process of adding a new action.
...
\begin{description}
    ...
    \item [CONNECT] Handle a remote client connecting: create a new
        ...
        data captured by the rule (\acronym{IP} address and hostname of the
    ...
    \item [MAIL\_BOUNCED] XXX WRITE THIS\@; INVOKES SAVE\_DATA ACTION\@.

    \item [MAIL\_DISCARDED] XXX WRITE THIS\@; INVOKES COMMIT ACTION\@.
    ...
    \item [MAIL\_QUEUED] This action represents Postfix picking a mail from
        ...
        and \daemon{cleanup} because it needs to deal with out of order log
        lines; see \sectionref{discarding cleanup log lines} for details.
        XXX EXPLAIN THIS BETTER\@.

    \item [MAIL\_SENT] XXX WRITE THIS\@; INVOKES SAVE\_DATA ACTION\@.
...
need to extend the list of valid combinations described in \sectionref{out
of order log lines} if the new action creates a different set of acceptable
...
mail server used for development does not utilise either the \daemon{lmtp}
or \daemon{virtual} delivery agents, so this parser does not have rules to
...
differs significantly from the \daemon{smtp} or \daemon{local} delivery
...
\begin{itemize}
    ...
    \item Improve the heuristics used in \sectionref{timeouts during data
        phase}, or develop another solution, to avoid incorrectly leaving a
