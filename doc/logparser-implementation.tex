...
\begin{eqlist}
    ...
    \item [postfix\_action] The action Postfix must have taken to generate
        ...
        \sectionref{list of postfix actions}.
...
\begin{description}
    ...
    \item [CONNECT] Handle a remote client connecting: create a new state
        table entry (indexed by \daemon{smtpd} \gls{pid}) and save both the
    ...
    \item [MAIL\_QUEUED] This action represents Postfix picking a mail from
        ...
        lines; see \sectionref{discarding cleanup lines} for details.
...
\subsection{Mail enters the system}
...
submission via \daemon{postdrop}, by \gls{SMTP}, by re-injection due to
...
first be correlated by the \daemon{smtpd} \gls{pid}, then transition to
...
during the data phase \sectionref{timeouts-during-data-phase}).  If the
...
The DATA phase of the \gls{SMTP} conversation is where the headers and body
...
\subsection{Discarding cleanup lines}
