...
\begin{boldeqlist}
    ...
    \item [server\_ip] The \acronym{IP} address of the server: the local
    ...
    \item [client\_ip] The client \acronym{IP} address: the remote address
...
\begin{enumerate}
    ...
    \item How long it takes to parse each log file (\graphref{parsing time
        vs file size vs number of log lines graph}).

    \item The number of rules tried when recognising log lines
        (\graphref{Mean number of rules tried per log line}); the framework
    ...
    \item The number of log lines and rules tried for each Postfix
        component (\graphref{Mean number of rules tried per log line for
...
\begin{description}
    ...
    \item [Pre-process the file.]  The new algorithm leverages the
        ...
        commonly occurring variable terms (e.g.\ email addresses,
