...
\begin{enumerate}
    ...
    \item If the mail is rejected, save all data to the database and
        delete the connection entry from the state table.

    \item Follow the progress of the accepted mail until it is either
        delivered, bounced, or deleted, then save all data to the database,
        and delete the mail's entry from the state table.
...
explicit log lines of its own; it is explained in \sectionref{Re-injected
mails}.  Internally generated mails lack any explicit origin in Postfix
2.2.x and must be detected using heuristics as described in
...
\begin{enumerate}
    ...
    \item This is followed by rejection of sender address, recipient
        ...
        etc.\ \flowchart{REJECTION}{4}; acceptance of one or more mails
...
\begin{enumerate}
    ...
    \item Sometimes mail is re-injected and the child mail needs to be
        tracked by the parent mail \flowchart{TRACK}{10};
        \sectionref{tracking re-injected mail} discusses this in detail.
    ...
    \item Eventually the mail will be delivered, bounced, or deleted by the
        ...
        mails is described in \sectionref{tracking re-injected mail}.
