...
RESCUE SOME OF IT\@.  IT SHOULD HAVE A BLURB, FOLLOWED BY A PREVIEW OF THE
CHAPTER\@.
...
responds to events (mouse clicks in a GUI program, log lines being
recognised in \parsername{}) and invokes the correct action.
...
presumably rare.  XXX WOULD THIS PARAGRAPH BE BETTER AS A LIST\@?
...
\begin{boldeqlist}
    ...
    \item [smtp\_code] The \acronym{SMTP} code associated with the log
        ...
        represented by the log line.  XXX GIVE AN EXAMPLE LOG LINE\@.
...
surprisingly, \action{UNINTERESTING}: this action does nothing when
...
rules specifying the \action{UNINTERESTING} action recognise log lines that
...
recognised by a single rule and handled by the \action{CONNECT} action.
...
and generate a regex to match each group of log lines.  There are 4 steps
in the algorithm:
...
\begin{description}
    ...
    \item [Build regexes.]  The words are reassembled to produce a regex
        ...
        but once again 1\% of log lines is a good starting point.  The new
        regexes are displayed for the user to add to the database, either
        as new rules or merged into the regexes of existing rules; the
        counter for each regex is also displayed, giving the user an
...
XXX MERGE logs2regexes WITH check-regexes.  CHECK IF SLCT DOES THIS STEP\@.

A second utility is also provided that reads a list of new regexes and the
input given to the first utility.  It tries to match each input log line
with each regex, counting the number of log lines that match each regex,
warning the user if an input log line is matched by more than one regex,
and additionally warning if an input log line is not matched by any regex.
It displays a summary of how many input log lines each regex matched,
comparing it to the expected number of matches; this provides the user with
an easy method of checking if the regexes produced by the first utility are
correctly matching the input log lines they are based upon.
...
This section has presented \parsername{}, the parser implemented for this
...
and the conveniences it offers to users.  The data collected by the
framework is described, as are the ways in which various optimisations can
be disabled to demonstrate their effect, and the debugging options the
