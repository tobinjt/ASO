...
\begin{enumerate}
    ...
    \item The parsers reviewed all produce a report of varying complexity
        and detail, whereas \acronym{PLP} does not; it extracts data and
...
\acronym{SMTP} code, sender, recipient, size, message ID\@.
...
available.  It produces a \acronym{CSV} file and either a MySQL
(\urlLastChecked{http://www.mysql.com/}{2008/11/23}) or Berkeley DB
(\urlLastChecked{http://www.oracle.com/database/berkeley-db/index.html}{2008/11/23})
database.  The decision to support both \acronym{CSV} and Berkeley DB
...
date, client hostname and \acronym{IP} address, server \acronym{IP}
address, sender and recipient addresses, \acronym{SMTP} code, and size (for
...
Handling of multiple recipients, \acronym{SMTP} codes, or remote
...
preprocessing before parsing by \acronym{LMA}.  The \numberOFlogFILES{}
preprocessed test log files were parsed without complaint by \acronym{LMA},
although it produced 32 entries in the output \acronym{CSV} file for every
...
\begin{itemize}
    ...
    \item DAILY-REJECT considers all \acronym{SMTP} codes except
        \texttt{250} to be rejections; this includes invalid \acronym{SMTP}
        codes such as \texttt{0} and \texttt{deferred}, so the awk command
        does too.  These invalid \acronym{SMTP} codes are most likely
        present because of incorrect parsing by \acronym{LMA}.
...
\begin{enumerate}
    ...
    \item
        \urlLastChecked{http://www.thesawmill.co.uk/formats/postfix.html}{2008/11/23}
        \newline{} Fields extracted: from, to, server, UID, relay, status,
        number of recipients, origin hostname, origin \acronym{IP} address,
    ...
    \item
        \urlLastChecked{http://www.thesawmill.co.uk/formats/postfix_ii.html}{2008/11/23}
        \newline{} Fields extracted: from, to, RBL list, client hostname,
        and client \acronym{IP} address.  It also counts the number and
        total size of all mails delivered.  

    \item
        \urlLastChecked{http://www.thesawmill.co.uk/formats/beta_postfix.html}{2008/11/23}
        \newline{} Fields extracted: from, to, client hostname, client
        \acronym{IP} address, relay hostname, relay \acronym{IP} address,
...
rejection is caused by a \acronym{DNSBL} check (\texttt{RBL list} in the
list of fields).
...
admitted defeat.  Using the \acronym{CLI} rather than the web interface was
...
\urlLastChecked{http://www.splunk.com/}{2008/11/23}, but poor organisation
...
timestamps for each log line, \acronym{SMTP} code, and message ID), though
...
resulting database).  The full power of \acronym{SQL} is available when
...
\urlLastChecked{http://httpd.apache.org/}{2008/11/23}.} format log files,
...
The Debian project (\urlLastChecked{http://www.debian.org/}{2008/11/23})
