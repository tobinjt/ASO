...
\begin{description}
    ...
    \item [MySQL] MySQL is a widely used, open source, relational database.
        \acronym['s]{LMA} MySQL support was not tested because the database
        schema used by \acronym{LMA} is not documented, so the required
...
\acronym{LMA} requires major changes to the parser code to parse new log
...
\acronym{regex}, so parsing new log lines requires modifying an existing
\acronym{regex} or carefully inserting a new block in the correct place;
extracting extra data would require modifying multiple blocks, \regexes{},
or both.  \regexglossarydescription{}
...
codes, delivery delays, and message-id.  \parsername{} does not make the
full text of the log line available; a few minutes work could add this to
...
\parserblurb{%
    ...
    the most versatile log analysis software available today.  Lire not
    only keeps you informed about your HTTP, FTP, and mail traffic, it also
    ...
    have pioneered many new services and features.  Lire is a total
    solution for your log analysis needs.
}
...
Similar to AWStats and Logrep (\sectionref{Logrep review}), Lire attempts
to correlate log lines by queueid, but not by \acronym{pid}, so the
...
fields are saved when the log line does not match the \acronym{regex},
though it appears that they will not overwrite existing data.  Most
notably, rejected delivery attempts are completely ignored, making it
unsuitable for the purposes of this project.  It does not try to address
any of the complications in \sectionref{complications} except for
correlating log lines by queueid.
...
% Added because extra spacing causes the new section to be badly
% positioned.
\newpage{}
