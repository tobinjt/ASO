...
\begin{enumerate}
    ...
    \item Only \parsername{} makes it possible to parse new log lines
        ...
        \parsernames{} architecture is described in \sectionref{why
    ...
    \item The parsers reviewed all produce a report of varying complexity
        ...
        of generating such reports (discussed in \sectionref{database as
        API}); some simple examples are given in \sectionref{motivation}.
...
described in \sectionref{parser efficiency}.  The data extracted by
\parsername{} is documented in \sectionref{connections table} and
\sectionref{results table}; that list is repeated here, without the
explanations:  server\_ip, server\_hostname, client\_ip, client\_hostname,
helo, queueid, start, end, warning, smtp\_code, sender, recipient, size,
message\_id, data, timestamp.

\subsection{Log Mail Analyser}

\label{prior art}

XXX LIST THE DATA NOT EXTRACTED BY EACH PARSER FROM HERE ON\@.
...
XXX UPDATE THE BLURBS FROM HERE ON\@.
