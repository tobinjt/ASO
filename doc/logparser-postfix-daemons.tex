...
\newglossaryentry{local}{name={local},type={postfix},description={
    ...
    (i.e.\ mail delivered on the server Postfix is running on); this
    includes alias expansion, processing of a user's \texttt{.forward}
    file, and delivery of the mail, whether it be to a user's mailbox or a
    program such as a mailing list manager.
...
\newglossaryentry{pickup}{name={pickup},type={postfix},description={
    ...
    \daemon{postdrop} and \daemon{sendmail,} passing the mail on to
    \daemon{cleanup} for further processing.
...
\newglossaryentry{postdrop}{name={postdrop},type={postfix},description={
    ...
    \daemon{pickup} and subsequent delivery by one or more of
    \daemon{lmtp}, \daemon{local}, \daemon{smtp}, or \daemon{virtual}.
...
\newglossaryentry{sendmail}{name={sendmail},type={postfix},description={
    A Postfix component that is compatible with the Sendmail
    (\urlLastCheckedNoParens{http://www.sendmail.org/}{2009/02/23}) mail
...
\newglossaryentry{virtual}{name={virtual},type={postfix},description={
    ...
    the \texttt{@}, whereas with \daemon{virtual} delivery the destination
    is determined by the entire email address.  E.g.\ if the server is
    responsible for both the \texttt{example.org} and \texttt{example.net}
    domains: \daemon{local} would deliver mail for
