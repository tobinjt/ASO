...
for each Postfix component; \graphref{Mean number of rules tried per log
line} shows the drop in the mean number of rules tried per log line for log
files containing a mail loop, and \graphref{Mean number of rules tried per
...
(see \graphref{Mean number of rules tried per log line}).  The performance
...
\begin{itemize}
    ...
    \item 9 mails whose initial log lines were not present in the log
        ...
        \sectionref{Re-injected mails} for details.  XXX CHECK IF THERE IS
