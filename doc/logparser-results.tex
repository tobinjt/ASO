...
XXX THINK ABOUT THE ORDERING OF TABLES AND GRAPHS\@; MAYBE REARRANGE THEM
SO THAT THEY APPEAR IN THE ORDER THEY ARE REFERENCED\@?

XXX SHOULD I USE $\backslash{}$clearpage{} OR SOMETHING TO AFFECT
...
WANT TO HAVE TEXT, THEN ALL GRAPHS AND TABLES, THEN THE NEXT SECTION\@; I
DO NOT LIKE THAT GRAPHS/TABLES SOMETIMES APPEAR BEFORE THEY ARE
REFERENCED\@.  I MIGHT NEED TO REDUCE THE HEIGHT OF THE GRAPHS TOO\@.
...
\graphref{Mails received per day} shows that, as expected, far more mails
are received on weekdays than at weekends.  The mail server in question is
a production mail server handling mail for a university department; the
...
noticeable in later graphs do not contribute to these figures.
...
parsed 10 times, and the parsing times from the 10 runs averaged.  Saving
...
disks the database is stored on.  The computer used for test runs was a
Dell Optiplex 745, shown in \tableref{Details of the computer used to
generate statistics}.
...
larger than average size (shown in \graphref{parsing time vs file size vs
number of log lines graph}); this is explained fully in
\sectionref{Explaining the peaks in log file size}.
...
XXX SHOULD THIS BE A SEPARATE SECTION OR INCLUDED IN THE PREVIOUS ONE\@?
MAYBE INCLUDE GRAPHS OF RULES PER FILE OR SOMETHING TO EXTEND THIS\@?

XXX REWRITE WHEN STATS RUNS HAVE FINISHED\@; REWRITE ALL THE REFERENCES
TOO\@.
...
username@domain}\newline{}  Normally this works well, but the forwarding
method in use at that time stripped the Delivered-To header from the mail,
breaking loop detection.

When the recipient in a mail loop is local, Postfix does not have to
deliver the mail to an external system, so mails are delivered as fast as
the disks can keep up, resulting in a huge volume of log lines.
...
\clearpage{} % XXX GRAPHS MOVE AROUND A LOT
...
\clearpage{} % XXX GRAPHS MOVE AROUND DURING REBUILDS
