...
servers would have much larger log files.  Note that \graphref{Mails
received per day} and \tableref{Number of mails received per day:
statistics} show the number of mails received by \acronym{SMTP} only; in
...
10\% recognise the vast majority of log lines, with the remaining log lines
...
frequently recognising rules before those that recognise less frequently.
...
shuffled ordering.  The optimal ordering (where the rules that recognise
log lines most frequently are tried first) shows a mean increase of
