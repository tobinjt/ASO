...
\Graphref{percentage increase of shuffled over normal} shows the percentage
increase of shuffled over normal, with the percentage increase of reverse
over normal following in \graphref{percentage increase of reversed over
normal}; the mean increases for different groupings of log files are given
in \tableref{Execution time increase for different rule orderings}.
Overall this optimisation provides a modest but worthwhile performance
increase of approximately 10\%, for a small investment in time and
programming.

\label{rule ordering graphs}

XXX SHOW NORMAL AS A REDUCTION FROM SHUFFLE, AND REVERSE AS AN INCREASE
OVER SHUFFLE\@.
...
anomalies and patterns are immediately obvious.  The
