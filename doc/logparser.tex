...
are 28 warnings produced, but because the parser errs on the side of
...
committal, ambiguity) for a problem which occurs only five times in
60721709 log lines.
...
the logs.  After parsing the 93 test log files the state tables contain 25
entries, breaking down into:

\begin{itemize}

    \item 1 connection which started only seconds before the logs ended
