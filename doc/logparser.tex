...
This complication is further complicated by the presence of out of order \daemon{cleanup}
lines: see section~\refwithpage{discarding cleanup lines} for details.
...
In the case where the \daemon{cleanup} line is still pending the algorithm
updates a global cache of queueids, adding the queueid and the timestamp
from the timeout line.  When the next \daemon{cleanup} line is parsed for
that queueid the cache will be checked, and the line will be deemed part of
the discarded mail and discarded if it meets the following requirements:
...
next \daemon{cleanup} line it would in some cases be mistaken.  Whether or
not the next \daemon{cleanup} line is discarded the queueid will be removed
from the cache of timeout queueids when the next \daemon{pickup} line
containing that queueid is parsed.
...
and if the queueid exists and the \daemon{cleanup} line passes the checks
listed above it will be discarded.
...
via sendmail appears later in the logfile than the \daemon{cleanup} line
...
\begin{itemize}
    ...
    \item There is less than a five second difference in the timestamps of
        the \daemon{cleanup} and \daemon{pickup} lines.
...
\begin{enumerate}
    ...
    \item Postfix is stopped or its configuration is reloaded.  When this
        ...
        connections state table must be committed and deleted from the
        state table.  The queueid state table is untouched.

    \item Postfix sometimes forcibly kills a \daemon{smtpd}, so the active
    ...
    \item Occasionally a \daemon{smtpd} will die or exit uncleanly, so the
        active connection must be committed and deleted from the
        connections state table.

\end{enumerate}

The above appear to cover all situations where a \daemon{smtpd} suddenly
stops logging.  In addition to removing an active connection the last
accepted mail may need to be discarded, as detailed in
...
\begin{description}
    ...
    \item [\daemon{local}:] Local delivery of bounce notification, or local
    ...
    \item [\daemon{smtp}:] Remote delivery of forwarded/tracked mail.
...
\glossary{name={ESMTP},description={
