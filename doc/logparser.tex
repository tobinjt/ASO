...
XXX MAYBE ADD A SECTION ABOUT OPTIMISATION OF THE DATABASE\@.
...
\begin{description}
    ...
    \item [rule\_order] is an efficiency measure.  This counter is
        ...
        in section~\ref{rule ordering for efficiency}.
...
XXX WRITE ABOUT DATABASE OPTIMISATION??
...
XXX THIS NEXT PARAGRAPH MIGHT NEED TO BE REWRITTEN\@.
...
took some time to arrive at, but turns out to be reasonably simple.
Examining the database after parsing logfile 22 shows 64 rules matched
during parsing, but the distribution of matching rules is extremely skewed,
as shown in graph~\ref{log22 distribution by rule}.  Graph~\ref{rule
distribution by program} shows rule distribution by program.  Comparing the
number of log lines per program and rules per program shows that
\textit{postfix/qmgr\/} has the highest number of log lines but has a low
number of rules to try, so the average execution time for a
\textit{postfix/qmgr\/} is quite low, reducing the overall average
execution time per log line.

XXX CREATE A GRAPH AVERAGE EXECUTION TIME PER LOG FILE\@.
...
\subsubsection{Log 22 distribution by program}
\label{log22 distribution by program}
\includegraphics{plot-log22-distribution-by-program.ps}

\subsubsection{Rule distribution by program}
\label{rule distribution by program}
\includegraphics{plot-log22-rule-count-by-program.ps}
