...
explicit logging smtpd or postdrop injected mails have.  Thus the source is
not immediately discernible from the log line in which the mail first
appears; from a strictly chronological reading of the logs it
\textit{usually\/} appears as if the child mail has appeared from thin air.
...
require an explicit source, as bugs in the parser are more likely to be
exposed.
...
submission via postdrop/sendmail,\footnote{Postfix provides a command that
is compatible with the Sendmail (\url{http://www.sendmail.org/ XXX CHECK})
mail submission program which all Unix commands which send mail depend on.}
by \SMTP{}, by re-injection due to forwarding, or internally generated by
Postfix.  Local submission is the simplest case: a queueid is assigned
