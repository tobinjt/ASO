...
which are applied to the lines.  Rules are solely concerned with
identifying a line and extracting data from it, whereas the algorithm's
task is to follow the journey each mail takes through Postfix, piecing the
data extracted by rules into a coherent whole, saving it in a useful and
...
the resulting data in a normalised, easy to use representation.
...
\input{stats-cached-discarded-include}
