...
evidenced by logs produced in normal operation (see graph~\refwithpage{rule
hits graph} for a vivid illustration of this).  


Enabling users to easily extend the parser to deal with new log lines is a
design imperative, and despite the resulting increase in complexity the job
is quite tractable, though it does raise interesting efficiency and
optimisation questions (answered in section~\refwithpage{rule efficiency}).
For the parser to be useful to the average user, parsing new log lines
produced by Postfix must be as simple a task as configuring Postfix to
produce those log lines was in the first place, otherwise users won't make
to effort to utilise the parser.  Providing a tool to ease the generation
of \regexes{} from unparsed log lines should help 
...
occurring log lines is a simple, albeit tiresome task, whereas dealing with
information which hasn't been logged, e.g.\ when delivery attempts are
aborted.  
