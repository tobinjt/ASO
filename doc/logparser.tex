...
\begin{itemize}
    ...
    \item The timestamp of the cleanup line must be within seven minutes of
        ...
        evidence suggests seven minutes is a good compromise between false
...
sections~\ref{tracking-re-injected-mail},~\ref{discarding cleanup
lines}~and~\ref{pickup logging after cleanup}), dealing with these lines
would introduce a large amount of complication (deferred warnings, deferred
committal, ambiguity) for a problem which occurs only four times in
60721709 log lines.

XXX WRITE THIS

The state table contains entries for mails not yet delivered when the
parser finishes execution.  Ideally all it should contain are mails which
are awaiting delivery after the period covered by the logs, though it may
also contain mails from the start of the logs whose initial entries are not
contained in the logs.  Any other entries are evidence of a failure in
parsing or an aberration in the logs.  After parsing the 93 test log files
the state table contains XXX entries, breaking down into:
...
\begin{itemize}
    ...
    \item 9 mails which had yet to be delivered due to repeated failures.

    \item 4 mails resulting from the out of order log entries described
        above.

    \item 9 mails whose initial log entries are not present in the logs.
