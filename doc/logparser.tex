...
complications).  The program fails to parse Postfix 2.2 or 2.3 log files;
indeed all it produces is a number of warning messages from the Perl
interpreter, without any other output, useful or otherwise.
...
XXX MAYBE REMOVE THE NEWPAGE BELOW WHEN I'VE SPLIT THE FLOW CHART\@.
...
\begin{itemize}
    ...
    \item If the mail is a child mail then it has already been tracked
        (action: commit; flowchart:~20): the data is cleaned up, the child
        is entered in the database (flowchart:~21), then deleted from the
