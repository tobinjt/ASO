...
their source.  The solution is that heuristics must be used to identify
them, and those heuristics are:

\begin{enumerate}

    \item The sender address will be <>.

    \item Neither \texttt{postfix/smtpd} nor \texttt{postfix/pickup} will
        have logged any messages associated with the mail, viz it was
        generated internally by Postfix, not accepted via SMTP or submitted
        locally by sendmail.
...
\subsubsection{Out of order log entries}

\label{out-of-order-log-entries}
