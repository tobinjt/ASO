...
% XXX CAN I MAKE THIS LOOK BETTER?  MAYBE USE [] LIKE \pageref{} DOES?
\newcommand{\urlLastCheckedNoParens}[3][ ]{%
    \url{#2}#1last checked #3%
}
...
\begin{itemize}
    ...
    \item XXX WHERE SHOULD I USE URLS AND WHERE SHOULD THEY BE BIBLIOGRAPHY
        ENTRIES\@?

    \item XXX REFERENCE FOR TRANSDUCTION\@.

    \item XXX PREVIOUS RESEARCH IN OVERLAPPING RULES\@?

    \item XXX BORROW SPINNING WHEEL BOOK\@; NEED TO COMMENT ON THE
        COMPUTATIONAL COMPLEXITY OF FA UNION AND INVERSION\@; MAYBE COMMENT
        ON COMPUTATIONAL COMPLEXITY OF PDA UNION, INVERSION, AND
        INTERSECTION\@?

    \item XXX BIB ENTRY FOR CHURCH-TURING\@.
