...
principle was written in a friendlier time. XXX CARL WANTS ME TO SUGGEST A
NEW PRINCIPAL AND POSSIBLY AN APPENDIX OF SPAM RELATED ISSUES\@.}  A
typical \SMTP{} conversation resembles the following (the lines starting
...
for some restrictions, e.g.\ XXX IMPROVE THIS EXPLANATION OR REMOVE IT \\
...
\newline checks whether the \IP{} address of the connected client matches
the left hand side of each line in the file and will return the right hand
side of the first matching line.  Other restrictions determine their result
in different ways, e.g.\ \texttt{reject\_rbl\_client rbl.example.com}
checks whether the \IP{} address of the client is present in the listed
\RBL{}\@.
...
XXX IMPROVE THE NEXT PARAGRAPH BASED ON COMMENTS FROM CARL\@.

XXX REFER TO THE RULES/PARSER SPLIT ELSEWHERE RATHER THAN EXPLAINING IT
AGAIN HERE\@.

XXX``CATEGORISE THE COMPLICATIONS SECTIONS IN WORDS HERE'' --- CARL\@.

XXX DISCUSS THE OUTPUT, OR LACK THEREOF, IN GREATER DETAIL, AFTER RERUNNING
\LMA{}\@.
...
complications discussed in sections~\refwithpage{complications}
and~\refwithpage{additional complications}.  The program fails to parse
Postfix 2.2 or 2.3 log files; indeed all it produces is a number of
warning messages from the Perl interpreter, without any other output,
useful or otherwise.  Parsing in \parsername{} is divided in two (see
...
produce any reports, it delegates that task to a separate, dedicated
program.
...
\regexs{}, but dynamic \regexs{} need to be explicitly compiled and cached.
...
\begin{description}
    ...
    \item [Pre-process the file]  The new algorithm leverages the knowledge
        ...
        create more accurate \regexs{}.  The new log lines are written to a
        temporary file, which all subsequent stages use instead of the
        original input file.
    ...
    \item [Build regexs]  Contiguous sequences of \texttt{.*} in the newly
        reassembled \regexs{} are collapsed to a single \texttt{.*}; any
        resulting duplicate \regexs{} are combined, and their counters
        ...
        number and type of input lines.}  The new \regexs{} are printed for
        the user to add to the database, either as new rules or merged into
        the \regexs{} of existing rules; the counter for each \regex{} is
        also printed, giving the user an indication of how many of the
        input lines that \regex{} should match.  Discarding \regexs{} will
        ...
        running the parser, including the new \regexs{}, with the same
        input which produced the original set of unparsed lines.

\end{description}

A second utility is also provided which reads a list of new \regexs{} and
...
user with an easy method of checking if the \regexs{} produced by the first
utility are correctly matching the input lines they are based upon.  A
future version of this utility will also group input lines by \regex{}, so
the user can tweak the \regexs{} if required.
...
XXX SPLIT THIS IMAGE INTO TWO\@; IT'S UNREADABLE NOW, IT'S SO BIG\@.
...
XXX ADD SOMETHING ABOUT FALSE POSITIVES AND NEGATIVES\@.
...
\begin{description}
    ...
    \item [AWStats] \textit{AWStats is a free powerful and featureful tool
        ...
        \newline  There does not seem to be an explanation of those fields
        in the documentation.  AWStats has no special purpose code to deal
        with the intricacies of Postfix logs, in fact it operates by
        ...
        generated by Postfix, and though it does correlate log lines by
        queueid, it does not deal with any of the other complications
    ...
    \item [Yet Another Advanced Logfile Analyser] \textit{yaala is a very
        ...
        cannot be used to extract data for external use, merely to create
        reports.
...
\begin{enumerate}
    ...
    \item GOTO 2.  XXX IMPROVE THIS, IT'S RUBBISH\@.
