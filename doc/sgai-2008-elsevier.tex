% $Id$
\documentclass[preprint,draft,numbers,1p]{elsarticle}
...
% Use eqlist instead of description lists to get lists with consistent
% indentation.
\usepackage{eqlist}

% Nicer citation lists - smaller spaces.
% Breaks, probably due to conflict with natbib.
%\usepackage{cite}
...
\ATN{}~\cite{atns}, used in Computational Linguistics for creating
...
\begin{table}[htbp]
    ...
    \begin{tabular}[]{ll}
        ...
        Attribute                 & Value                                                           \\
        \tablemiddleline{}%
        regex                     & {\footnotesize \verb!^connect from (__CLIENT_HOSTNAME__)\[(__CLIENT_IP__)\]$!}  \\
...
\input{build/include-timing-run-duration}, mean throughput
...
\begin{eqlist}
    ...
    \item [shuffled] Random ordering, intended to represent an unsorted
        rule set.  Note that the ordering will change every time the parser
...
\bibliographystyle{sgai-2008-elsevier}
