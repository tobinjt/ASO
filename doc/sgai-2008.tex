...
XXX MAYBE MERGE THE AUTHOR SECTIONS IF DESPERATE\@.
...
between regexes and FA, so regex overlap could be calculated by finding the
intersection of two FA XXX REFERENCE\@.  Perl 5.10 regexes are more
powerful than traditional regexes, e.g.\ it is possible to match correctly
nested and balanced brackets to an arbitrary depth:
\newline{}\tab{}\verb!/^[^<>]*(<(?:(?>[^<>]+)|(?1))*>)[^<>]*$/!\newline{}
...
detection would require calculating the intersection of two PA.\@  XXX
REFERENCE OR SOMETHING\@.

XXX OVERLAP DETECTION\@: L1 n L2 = E* --- ((E* --- L1) u (E* --- L2)).
CONTAINMENT CANNOT BE DETERMINED FOR CFGs, INTERSECTION IS POSSIBLE\@.
