...
XXX DISCUSS COMPLEXITY\@?  EVEN JUST MENTION THAT COMPLEXITY RISES WHEN
INCREMENTAL PARSING IS USED, BUT THAT EXTERNAL TOOLS COULD BE USED TO DEAL
WITH IT, AND THAT IT IS ENTIRELY DEPENDANT ON THE RULESET USED\@?

XXX MAYBE MERGE THE AUTHOR SECTIONS IF DESPERATE FOR SPACE\@.

XXX REVIEWER SUGGESTED COMPARING TO EXISTING WORK, BUT I DID NOT FIND ANY
SIMILAR EXISTING WORK\@.  SHOULD SEARCH AGAIN\@.

XXX OVERLAP DETECTION\@: L1 n L2 = E* --- ((E* --- L1) u (E* --- L2)).
PERL REGEXES ARE AT LEAST AS POWERFUL AS A PUSHDOWN AUTOMATA --- SEE
CHOMSKY'S HIERARCHY TO MAKE SURE\@.  CONTAINMENT CANNOT BE DETERMINED FOR
CFGs, INTERSECTION IS POSSIBLE\@.
...
\input{build/timing-run-include.tex}.  The computer was dedicated to the
task of gathering statistics from test runs, and was not used for any other
purpose while test runs were ongoing; any services not necessary for
...
approximately 10\%.  \showgraph{build/plot-hits}{Hits per rule}{rule hits
graph}
...
ruleset}.  \showgraph{build/plot-normal-vs-smaller}{Percentage parsing time
increase of maximum ruleset over minimum ruleset}{Percentage parsing time
increase of maximum ruleset over minimum ruleset}
...
\input{build/normal-vs-smaller-percentage-increase.tex} in parsing time for
