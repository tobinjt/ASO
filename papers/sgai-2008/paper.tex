...
\usepackage[style=altlist,number=none,toc=true]{glossary}
\makeglossary{}
...
\abstract{%
    ...
    parse new inputs, without requiring that user have an in-depth
    ...
    previously unknown class of input).  The architecture provides a
    framework which actions and rules fit into: this framework manages the
    parsing process, provides support functions for the actions, and
    automatically performs some optimisations which improve the speed of
    parsing.
}
...
with the huge variety of log lines produced by Postfix~\cite{postfix}, the
mail server in use: 
...
improved independently of the other.
...
the distribution of rules to actions is very skewed, as shown in
...
handles rejections; it's followed by \texttt{INFO}, the action responsible
for handling informative inputs: these are log lines that supplement the
data gathered from other log lines (such as rejections), but that don't by
themselves represent interesting events.  The third most common action is,
perhaps surprisingly, \texttt{IGNORE}: there are many log lines which are
not of interest to the parser, e.g.\ warning messages about DNS mismatches,
or log lines lacking a way to identify which data structure they should be
associated with.\footnote{Postfix allocates a queueid (explained in the
...
particular data structure.}  The parser warns about every unparsed log
...
log lines to be parsed without any effects.  The remaining actions have
only one or two associated rules: in some cases this is because there will
only ever be one type of input for that action, e.g.\ all log lines showing
...
during parsing.\footnote{\label{complications footnote}The complications
which arose during development of the parser will be documented in detail
...
\begin{description}
    ...
    \item [Partially context-sensitive actions:]  XXX IS THIS THE CORRECT
        ...
        information, XXX IS THE EXAMPLE NECESSARY\@?  E.g.\ in the Postfix
        parser when a client disconnects if there have been no useful log
        lines for that client the data is discarded, rather than trying to
        store empty records in the database.
    ...
    \item [Transduction:]  The parser can be thought of as implementing
        ...
        it to another form (a database).  Other implementations are free to
        choose the output format most suitable for their needs.
    ...
    \item [Closer to \NLP{} than fixed grammar:]  Unlike traditional
        ...
        alteration, in fact such alteration is expected.  XXX NEED MORE
        HERE\@.
...
execution time for the remaining 9 runs averaged.  XXX DO I NEED TO EXPLAIN
WHY\@?  The first run is discarded for two reasons:
